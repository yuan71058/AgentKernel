...
默认情况下，前端只收到自己发起的 run 的事件（通过 `session.send` 自动创建的 `forward_run_events`）。

如果需要接收其他连接发起的事件（多前端场景），手动调用：

```json
{ "command": "events.subscribe", "request_id": "r_sub", "session_id": "my_session", "payload": { "since_seq": 0 } }
```

> **注意**：单前端场景不要调用 `events.subscribe`，否则同一事件会推送两次（run 级 + session 级）。
...
```typescript
{
  session_id: string;
  subscribed: boolean;
  already_subscribed: boolean;  // 是否已经订阅过
  since_seq: number;
  current_seq: number;          // 当前最新序列号
  replayed: number;             // 补拉的事件数量
}
```
...
如果前端 B 同时自己也发起了 `session.send`，`events.subscribe` 会导致同一事件推送两次。解决方案：

- 前端 B 发送消息前调用 `events.unsubscribe` 取消 session 订阅
- 或者前端 B 只在"观察模式"下使用 `events.subscribe`，不自己发消息
...
| `events.subscribe` | 订阅实时事件 | 必填 | `since_seq?` |
